...
{
    private const string projectId = "gamenaughty-c7866-default-rtdb.asia-southeast1"; // You can find this in your Firebase project settings
    private const string secret = "VJt8twWw3EYZYr4R3aXe5uIlPz9SVKCR82rFEeEy";
    ...
    public static void PostScore(User user, string userId,  string idToken , int score)
    {
        user.Score = score;
        RestClient.Put<User>($"{databaseURL}users/{userId}.json?auth={idToken}", user).Then(response =>
        {
    ...
    {
        RestClient.Get($"{databaseURL}users.json?auth={secret}").Then(response =>
        {
